...
- **Critical Security Fix - Cross-User Data Leakage Resolved (October 24, 2025)**: Fixed critical security vulnerability where TanStack Query cache was persisting data across user sessions. Issue: Users logging in as Account B would see Account A's cached financial data (transactions, budgets, accounts, etc.) until hard refresh. Root cause: `staleTime: Infinity` cache configuration with no cache clearing on login/logout. Solution: Implemented comprehensive cache clearing with `queryClient.cancelQueries()` + `queryClient.clear()` on all authentication boundaries (login, logout, register, auth failure). Multi-layered security ensuring complete data privacy between users. All auth functions properly abort in-flight requests and clear cache before state transitions.
...
- **Enhanced Smart Goal Matching with Indonesian Language Support (August 2025)**: Added two new scoring criteria to improve accuracy for Indonesian users: Language Adaptation Score (0-10pts) with comprehensive Indonesian keyword patterns, transaction pattern recognition (nabung, dp, cicilan, etc.), and goal name variations; Behavioral Pattern Score (0-5pts) with user behavior learning, amount pattern analysis, and temporal intelligence for weekend/month-end patterns. Redistributed scoring system (Account:35pts, Keywords:25pts, Context:15pts, AI:10pts, Language:10pts, Behavioral:5pts) maintaining 100pts total while significantly improving cultural relevance and personalized matching accuracy
